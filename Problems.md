...
**Симптом:** idat.exe продолжает висеть в памяти (до 3.5+ GB RAM) даже после того, как Diaphora-скрипт полностью завершил экспорт. `subprocess.run()` / `proc.communicate()` никогда не получают return code, потому что процесс не умирает.

**Факт:**
- SQLite создан, 1469 функций экспортировано
- `*-crash` файл создан и не удалён (Diaphora не очистил)
- idat.exe висит с 3.6GB RAM, процесс не завершается
- `taskkill /f` единственный способ остановить

**Причина:** IDA headless (`idat.exe -A`) после выполнения `-S` скрипта (diaphora_headless.py) не вызывает `exit()` или `q()`. Возможно:
1. Diaphora_ida.main() не завершает IDA после export()
2. Исключение в export() (например, падение на `save_callgraph`) прерывает нормальный shutdown
3. IDA ждёт чего-то (поток, GUI callback) даже в headless режиме

**Доказательство:**
- После `run_export()` с Popen + communicate(timeout=3600), процесс не завершается до таймаута
- После `taskkill /f /pid` — файлы валидны, экспорт успешен
- WAL перестал расти за 10 минут до убийства — экспорт реально завершён

**Нужно:**
1. Добавить в `_diaphora_headless.py` принудительный `idaapi.q("Diaphora export complete")` или `sys.exit(0)` после `diaphora_ida.main()`
2. В `run_export()` добавить watchdog: если WAL не растёт >5 мин и SQLite валидна — считать экспорт завершённым и убить процесс
3. Либо перейти на запуск idat.exe в режиме `-B` (batch, без GUI вообще)
