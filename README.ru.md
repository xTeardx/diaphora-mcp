...
- **Security triage**: поиск security-релевантных сигналов по ключевым словам и эвристикам; результат требует ручной проверки
...
`idalib-mcp` — headless backend внутри `ida-pro-mcp`, а не отдельный Diaphora tool.
...
Полный пошаговый разбор реальной сессии Diaphora MCP — **[examples/basic-session.ru.md](examples/basic-session.ru.md)**.
...
- [Разбор реальной сессии Diaphora MCP](examples/basic-session.ru.md) — полный транскрипт с JSON-запросами и ответами сервера на каждом шаге.
...
Воспроизводимый unit suite не требует IDA Pro или бинарных fixtures. Экспорт и diff через IDA требуют локальной установки IDA Pro и Diaphora.
